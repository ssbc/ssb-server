...
function parseDhtAddress (addr) {
  ...
  return {host: seed + ':' + remoteId, port: 0, key: remoteId}
}
// #endregion
  ...
  init: function (server, config) {
    ...
    stateFile.get(function (err, ary) {
      ...
      server.emit('log:info', [
        'SBOT',
        '' + peers.length + ' peers loaded from',
        gossipJsonPath
      ])
    })
    ...
    var gossip = {
      ...
      get: function (addr) {
        ...
        // #region MODIFIED
        else if (isDhtAddress(addr)) getPeer(parseDhtAddress(addr))
        // #endregion
      ...
      connect: valid.async(function (addr, cb) {
        ...
        if (!ref.isAddress(addr.address)) { addr = ref.parseAddress(addr) }
        // #region MODIFIED
        if (isDhtAddress(addr)) {
          addr = parseDhtAddress(addr)
        } else if (typeof addr === 'string') {
          addr = ref.parseAddress(addr)
        } if (!addr || typeof addr !== 'object') { return cb(new Error('first param must be an address')) }
        // #endregion

        if (!addr.address) { if (!addr.key) return cb(new Error('address must have ed25519 key')) }
      ...
      add: valid.sync(function (addr, source) {
        if (isObject(addr)) {
          ...
          var _addr
          if (isDhtAddress(addr)) {
            _addr = parseDhtAddress(addr)
          } else if (typeof addr === 'string') {
            _addr = ref.parseAddress(addr)
          } else if (!addr || addr.source !== 'dht') {
            if (!ref.isAddress(addr)) { throw new Error('not a valid address:' + JSON.stringify(addr)) }
          }
          _addr = ref.parseAddress(addr)
          if (!_addr || _addr.source !== 'dht') throw new Error('not a valid address:' + addr)
          _addr.address = addr
          addr = _addr
          // #endregion
        }
        if (!ref.isAddress(addr.address) /* && !ref.isAddress(addr) */) { throw new Error('not a valid address:' + JSON.stringify(addr)) }
      ...
      rpc.on('closed', function () {
        ...
        server.emit('log:info', [
          ...
          [
            ...
            peer.state,
            'for',
            (new Date() - peer.stateChange) / 1000,
            'seconds'
          ].join(' ')
        ])
