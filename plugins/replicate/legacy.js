...
module.exports = function (sbot, notify, config) {
  ...
  function localPeers () {
    ...
    sbot.gossip.peers().forEach(function (e) {
      if (e.source === 'local') { request(e.key) }
    })
  }
    ...
    function next () {
      ...
      pull(
        ...
        drain = pull.drain(function (upto) {
          ...
          replicate(upto, function (ignoredErr) {
            if (ignoredErr) {
              // error can be safely ignored, we just want to abort the stream
            }

            drain.abort()
          })
        }, function (err) {
          if (err && err !== true) { sbot.emit('log:error', ['replication', rpc.id, 'error', err]) }
        })
      )
    }
  })
