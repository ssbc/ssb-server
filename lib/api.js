var DeferSource = require('pull-defer/source')
  ...
  each(manifest, function (_, name) {
    if(name === 'createHistoryStream') {
      ...
      api.createHistoryStream = function (opts) {
        ...
        if(opts.id === this.id)
          return ssb.createHistoryStream(opts)

        var defer = DeferSource()
        var blocked

        pull(
          ssb.links({source: opts.id, dest: this.id, rel: 'contact', type: 'feed', values: true}),
          pull.drain(function (op) {
           blocked = op.value.content.flag != null
          }, function (err) {
            if(!blocked)
              defer.resolve(ssb.createHistoryStream(opts))
            else
              defer.resolve(ssb.createHistoryStream({id: null, seq: 0}))
          })
        )
        return defer
      }
