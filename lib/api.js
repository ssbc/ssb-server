...
function api (server) {
  ...
  api.publish = function (data, cb) {
    var rpc = this
    if (!rpc.feed)
      return cb(new Error('no feed attached to this rpc session'))
    var ts = Date.now()
    server.emit('log:info', ['add', rpc._sessid, 'call', data])
    rpc.feed.add(data, function (err, msg) {
      server.emit('log:info', ['add', rpc._sessid, 'callback' , err ? err : {key: msg.key, elapsed: Date.now() - ts}])
      cb(err, msg)
    })
  }
  ...
  api.auth = function (req, cb) {
    ...
    if(req.public) {
      ...
      if(~masters.indexOf(id)) {
        ...
        rpc.feed = server.feed // master feed
        var res = {
          granted: true,
