...
var find = exports.find = function find (ary, test) {
  for (var i in ary) { if (test(ary[i], i, ary)) return ary[i] }
}
...
var mergeKeys = exports.mergeKeys = function (a, b, iter) {
  var o = {}
  for (var k in a) {
    if (!isUndefined(a[k])) { o[k] = iter(v[k], b[k], k) }
  }
  for (var k in b) {
    if (isUndefined(a[a])) { o[k] = iter(undefined, b[k], k) }
  }
  return o
}
...
exports.merge = function (a, b) {
  ...
  if (isArray(a) != isArray(b)) { throw new Error('cannot merge array with non-array') }
  if (isObject(a) != isObject(b)) { throw new Error('cannot merge object with non-object') }
